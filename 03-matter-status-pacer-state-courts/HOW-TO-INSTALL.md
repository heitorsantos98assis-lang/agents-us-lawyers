# 03-matter-status-pacer-state-courts — Claude Code Agent

A specialist subagent from the **57 Agents for US Lawyers** bundle (Bravy / ASV Digital).

## What this agent does

See `03-matter-status-pacer-state-courts.md` for the full description, frameworks, reference tables, and operating flow.
...
```bash
unzip 03-matter-status-pacer-state-courts.zip
```
...
cp 03-matter-status-pacer-state-courts.md .claude/agents/
...
```
Use the matter-status-pacer-state-courts subagent to ...
```

## Verifying installation

```bash
ls .claude/agents/   # should list 03-matter-status-pacer-state-courts.md
# OR
...
```bash
rm .claude/agents/03-matter-status-pacer-state-courts.md
# OR
rm ~/.claude/agents/03-matter-status-pacer-state-courts.md
```
