# 02-deadline-calendaring-frcp-state — Claude Code Agent

A specialist subagent from the **57 Agents for US Lawyers** bundle (HL).

## What this agent does

See `02-deadline-calendaring-frcp-state.md` for the full description, frameworks, reference tables, and operating flow.
...
```bash
unzip 02-deadline-calendaring-frcp-state.zip
```
...
cp 02-deadline-calendaring-frcp-state.md .claude/agents/
...
```
Use the deadline-calendaring-frcp-state subagent to ...
```

## Verifying installation

```bash
ls .claude/agents/   # should list 02-deadline-calendaring-frcp-state.md
# OR
...
```bash
rm .claude/agents/02-deadline-calendaring-frcp-state.md
# OR
rm ~/.claude/agents/02-deadline-calendaring-frcp-state.md
```
